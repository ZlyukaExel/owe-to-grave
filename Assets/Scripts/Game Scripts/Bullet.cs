...
public class Bullet : NetworkBehaviour
{
    ...
    [Header("NPC Trigger")]
    [SerializeField]
    private float ncpTriggerRadius = 5;
    ...
    private HashSet<Transform> alreadyTriggeredNpc = new();
    ...
    private void MoveBullet()
    {
        ...
        // Trigger NPC if bullet near
        foreach (
            var hit in Physics.SphereCastAll(
                transform.position,
                ncpTriggerRadius,
                transform.forward,
                step,
        ...
        {
            if (alreadyTriggeredNpc.Contains(hit.transform))
                continue;
