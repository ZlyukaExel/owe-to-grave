...
namespace Mirror
{
    ...
    public static class Prediction
    {
        ...
        public static T CorrectHistory<T>(
            ...
            int afterIndex
        ) // index of the 'after' value so we don't need to find it again here
            where T : PredictedState
        {
            ...
            after.positionDelta = Vector3.Lerp(
                Vector3.zero,
                after.positionDelta,
                (float)multiplier
            );
            after.velocityDelta = Vector3.Lerp(
                Vector3.zero,
                after.velocityDelta,
                (float)multiplier
            );
            after.angularVelocityDelta = Vector3.Lerp(
                Vector3.zero,
                after.angularVelocityDelta,
                (float)multiplier
            );
            // Quaternions always need to be normalized in order to be a valid rotation after operations
            after.rotationDelta = Quaternion
                .Slerp(Quaternion.identity, after.rotationDelta, (float)multiplier)
                .normalized;
