...
namespace Mirror
{
    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
        ...
        void ServerDirtyMasks_Broadcast(
            out ulong ownerMaskReliable,
            out ulong observerMaskReliable,
            out ulong ownerMaskUnreliableBaseline,
            out ulong observerMaskUnreliableBaseline,
            out ulong ownerMaskUnreliableDelta,
            out ulong observerMaskUnreliableDelta
        )
        {
            // clear
            ownerMaskReliable = 0;
                ...
                else if (component.syncMethod == SyncMethod.Hybrid)
                {
                    // UNRELIABLE DELTAS ///////////////////////////////////////
                    {
                        ...
                        if (component.syncMode == SyncMode.Observers)
                        {
                            ...
                            // observers which aren't the owner.
                            if (dirty)
                                observerMaskUnreliableDelta |= nthBit;
                        }
                    }
                    // UNRELIABLE BASELINE /////////////////////////////////////
                    {
                        ...
                        if (component.syncMode == SyncMode.Observers)
                        {
                            ...
                            // observers which aren't the owner.
                            if (dirty)
                                observerMaskUnreliableBaseline |= nthBit;
                        }
                    }
                    ////////////////////////////////////////////////////////////
                }
            ...
            for (int i = 0; i < components.Length; ++i)
            {
                ...
                if (isOwned && component.syncDirection == SyncDirection.ClientToServer)
                {
                    ...
                    else if (component.syncMethod == SyncMethod.Hybrid)
                    {
                        ...
                        // baseline sync runs @ 1 Hz (netmanager configurable).
                        // only consider dirty bits, ignore syncinterval.
                        if (component.IsDirty_BitsOnly())
                            dirtyMaskUnreliableBaseline |= nthBit;

                        // delta sync runs @ syncInterval.
                        // this allows for significant bandwidth savings.
                        if (component.IsDirty())
                            dirtyMaskUnreliableDelta |= nthBit;
                    }
                }
            }
        }
        ...
        internal void SerializeServer_Spawn(
        ...
        )
        {
            ...
            if (observerMaskUnreliableDelta != 0)
                Compression.CompressVarUInt(
                    observersWriterUnreliableDelta,
                    observerMaskUnreliableDelta
                );

            if (ownerMaskUnreliableBaseline != 0)
                Compression.CompressVarUInt(
                    ownerWriterUnreliableBaseline,
                    ownerMaskUnreliableBaseline
                );
            if (observerMaskUnreliableBaseline != 0)
                Compression.CompressVarUInt(
                    observersWriterUnreliableBaseline,
                    observerMaskUnreliableBaseline
                );
            ...
            if (
                (
                    ownerMaskReliable
                    | observerMaskReliable
                    | ownerMaskUnreliableBaseline
                    | observerMaskUnreliableBaseline
                    | ownerMaskUnreliableDelta
                    | observerMaskUnreliableDelta
                ) != 0
            )
            {
                for (int i = 0; i < components.Length; ++i)
                {
                    ...
                    bool observersDirtyUnreliableBaseline = IsDirty(
                        observerMaskUnreliableBaseline,
                        i
                    );
                    ...
                    if (ownerDirtyReliable || observersDirtyReliable)
                    {
                        ...
                        using (NetworkWriterPooled temp = NetworkWriterPool.Get())
                        {
                            ...
                            if (ownerDirtyReliable)
                                ownerWriterReliable.WriteBytes(
                                    segment.Array,
                                    segment.Offset,
                                    segment.Count
                                );
                            if (observersDirtyReliable)
                                observersWriterReliable.WriteBytes(
                                    segment.Array,
                                    segment.Offset,
                                    segment.Count
                                );
                    ...
                    if (ownerDirtyUnreliableDelta || observersDirtyUnreliableDelta)
                    {
                        using (NetworkWriterPooled temp = NetworkWriterPool.Get())
                        {
                            ...
                            if (observersDirtyUnreliableDelta)
                                observersWriterUnreliableDelta.WriteBytes(
                                    segment.Array,
                                    segment.Offset,
                                    segment.Count
                                );
                    ...
                    if (
                        unreliableBaseline
                        && (ownerDirtyUnreliableBaseline || observersDirtyUnreliableBaseline)
                    )
                    {
                        using (NetworkWriterPooled temp = NetworkWriterPool.Get())
                        {
                            ...
                            if (ownerDirtyUnreliableBaseline)
                                ownerWriterUnreliableBaseline.WriteBytes(
                                    segment.Array,
                                    segment.Offset,
                                    segment.Count
                                );
                            if (observersDirtyUnreliableBaseline)
                                observersWriterUnreliableBaseline.WriteBytes(
                                    segment.Array,
                                    segment.Offset,
                                    segment.Count
                                );
        ...
        // serialize Reliable and Unreliable components in one iteration.
        // having two separate functions doing two iterations would be too costly.
        internal void SerializeClient(
            NetworkWriter writerReliable,
            NetworkWriter writerUnreliableBaseline,
            NetworkWriter writerUnreliableDelta,
            bool unreliableBaseline
        )
        {
            // ensure NetworkBehaviours are valid before usage
            ...
            for (int i = 0; i < components.Length; ++i)
            {
                ...
                if (IsDirty(mask, i))
                {
                    ...
                    if (comp.syncDirection == SyncDirection.ClientToServer)
                    {
                        ...
                        if (!comp.Deserialize(reader, initialState))
                            return false;
        ...
        internal NetworkIdentitySerialization GetServerSerializationAtTick(
            int tick,
