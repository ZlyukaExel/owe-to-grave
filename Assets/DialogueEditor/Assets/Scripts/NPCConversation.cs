    ...
    public class NPCConversation : MonoBehaviour
    {
        ...
        private void ReconstructEditableConversation(EditableConversation conversation)
        {
            ...
            for (int i = 0; i < allNodes.Count; i++)
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < allNodes[i].Connections.Count; j++)
                    {
                        ...
                        {
                            EditableSpeechNode speech = conversation.GetSpeechByUID(
                                allNodes[i].Connections[j].NodeUID
                            );
                            (allNodes[i].Connections[j] as EditableSpeechConnection).Speech =
                                speech;
                        }
                        else if (allNodes[i].Connections[j] is EditableOptionConnection)
                        {
                            EditableOptionNode option = conversation.GetOptionByUID(
                                allNodes[i].Connections[j].NodeUID
                            );
                            (allNodes[i].Connections[j] as EditableOptionConnection).Option =
                                option;
                        }
                    }
                }
            ...
            for (int i = 0; i < allNodes.Count; i++)
            {
                ...
                {
                    if (
                        thisNode.Connections[j].ConnectionType
                        == EditableConnection.eConnectiontype.Speech
                    )
                    {
                        (thisNode.Connections[j] as EditableSpeechConnection).Speech.parents.Add(
                            thisNode
                        );
                    }
                    else if (
                        thisNode.Connections[j].ConnectionType
                        == EditableConnection.eConnectiontype.Option
                    )
                    {
                        (thisNode.Connections[j] as EditableOptionConnection).Option.parents.Add(
                            thisNode
                        );
                    }
                }
            ...
            for (int i = 0; i < ec.Parameters.Count; i++)
            {
                if (ec.Parameters[i].ParameterType == EditableParameter.eParamType.Bool)
                {
                    ...
                    BoolParameter boolParam = new BoolParameter(
                        editableParam.ParameterName,
                        editableParam.BoolValue
                    );
        ...
        public void CopyParamActions(EditableConversationNode editable, ConversationNode node)
        {
            ...
            {
                if (
                    editable.ParamActions[i].ParamActionType
        ...
        private void ReconstructTree(
            EditableConversation ec,
            Conversation conversation,
            Dictionary<int, SpeechNode> dialogues,
        ...
        )
        {
            ...
            for (int i = 0; i < editableSpeechNodes.Count; i++)
            {
                ...
                for (int j = 0; j < editableConnections.Count; j++)
                {
                    ...
                    if (
                        editableConnections[j].ConnectionType
                        == EditableConnection.eConnectiontype.Speech
                    )
                    {
        ...
        private void CopyConnectionConditions(
        ...
        )
        {
            ...
            {
                if (
                    editableConditions[i].ConditionType
