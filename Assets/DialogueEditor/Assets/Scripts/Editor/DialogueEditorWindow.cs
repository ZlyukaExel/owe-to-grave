...
namespace DialogueEditor
{
    public class DialogueEditorWindow : EditorWindow
    {
        ...
        private const string UNAVAILABLE_DURING_PLAY_TEXT =
            "Dialogue Editor unavaiable during play mode.";
        ...
        private EditableConversationNode m_connectionDeleteParent,
            m_connectionDeleteChild;
        ...
        public static bool OpenDialogue(int assetInstanceID, int line)
        {
            NPCConversation conversation =
                EditorUtility.EntityIdToObject(assetInstanceID) as NPCConversation;
        ...
        public void RecreateUI(EditableConversation conversation)
        {
            ...
            for (int i = 0; i < allNodes.Count; i++)
            {
                ...
                {
                    UISpeechNode uiNode = new UISpeechNode(
                        thisNode,
                ...
                {
                    UIOptionNode uiNode = new UIOptionNode(
                        thisNode,
                        new Vector2(thisNode.EditorInfo.xPos, thisNode.EditorInfo.yPos)
                    );
                    uiNodes.Add(uiNode);
        ...
        private void InitGUIStyles()
        {
            ...
            panelStyle.normal.background = DialogueEditorUtil.MakeTexture(
                10,
                10,
                DialogueEditorUtil.GetEditorColor()
            );
            ...
            resizerStyle.normal.background =
                EditorGUIUtility.Load("icons/d_AvatarBlendBackground.png") as Texture2D;
        }
        ...
        protected void OnLostFocus()
        {
            ...
            bool keepOnWindow =
                ...
                && EditorWindow.focusedWindow.titleContent.text.Equals("Dialogue Editor");
        ...
        private void DrawMessageDuringPlay()
        {
            ...
            Vector2 textDimensions = GUI.skin.label.CalcSize(
                new GUIContent(UNAVAILABLE_DURING_PLAY_TEXT)
            );
            Rect textRect = new Rect(
                centerX - (textDimensions.x / 2),
                centerY - (textDimensions.y / 2),
                textDimensions.x,
                textDimensions.y
            );
        ...
        private void DrawConnections()
        {
            ...
            if (
                ...
                && CurrentlySelectedObject.Type == SelectableUI.eType.Connection
            )
            {
            ...
            if (CurrentlySelectedObject == null)
            {
                ...
                for (int i = 0; i < CurrentAsset.ParameterList.Count; i++)
                {
                    ...
                    CurrentAsset.ParameterList[i].ParameterName = GUILayout.TextField(
                        ...
                        EditableParameter.MAX_NAME_SIZE,
                        GUILayout.Width(paramNameWidth),
                        GUILayout.ExpandWidth(false)
                    );

                    if (CurrentAsset.ParameterList[i] is EditableBoolParameter)
                    {
                        EditableBoolParameter param =
                ...
                CurrentAsset.DefaultName = EditorGUILayout.TextField(
                    CurrentAsset.DefaultName,
                    GUILayout.MaxWidth(fieldWidth)
                );
                ...
                CurrentAsset.DefaultSprite = (Sprite)
                    EditorGUILayout.ObjectField(
                        CurrentAsset.DefaultSprite,
                        typeof(Sprite),
                        false,
                        GUILayout.MaxWidth(fieldWidth)
                    );
                ...
                CurrentAsset.DefaultFont = (TMPro.TMP_FontAsset)
                    EditorGUILayout.ObjectField(
                        CurrentAsset.DefaultFont,
                ...
                CurrentAsset.ContinueFont = (TMPro.TMP_FontAsset)
                    EditorGUILayout.ObjectField(
                        CurrentAsset.ContinueFont,
                        typeof(TMPro.TMP_FontAsset),
                        false,
                        GUILayout.MaxWidth(fieldWidth)
                    );
                ...
                CurrentAsset.EndConversationFont = (TMPro.TMP_FontAsset)
                    EditorGUILayout.ObjectField(
                        CurrentAsset.EndConversationFont,
                        typeof(TMPro.TMP_FontAsset),
                        false,
                        GUILayout.MaxWidth(fieldWidth)
                    );
            ...
            else
            {
                ...
                if (CurrentlySelectedObject.Type == SelectableUI.eType.Node)
                {
                    ...
                    if (selectedNode is UISpeechNode)
                    {
                        ...
                        if (
                            ...
                            && node.Connections[0] is EditableSpeechConnection
                        )
                        {
                            GUILayout.Label("Auto-Advance options", EditorStyles.boldLabel);
                            node.AdvanceDialogueAutomatically = EditorGUILayout.Toggle(
                            ...
                            if (node.AdvanceDialogueAutomatically)
                            {
                                ...
                                node.TimeUntilAdvance = EditorGUILayout.FloatField(
                                    "Dialogue Time",
                        ...
                        node.Audio = (AudioClip)
                            EditorGUILayout.ObjectField(node.Audio, typeof(AudioClip), false);
                        EditorGUILayout.Space();

                        GUILayout.Label("TMP Font", EditorStyles.boldLabel);
                        node.TMPFont = (TMPro.TMP_FontAsset)
                            EditorGUILayout.ObjectField(
                                node.TMPFont,
                                typeof(TMPro.TMP_FontAsset),
                                false
                ...
                else if (CurrentlySelectedObject.Type == SelectableUI.eType.Connection)
                {
                    ...
                    EditableConnection connection = (
                        CurrentlySelectedObject as SelectableUIConnection
                    ).Connection;
                    ...
                    {
                        if (
                            CurrentAsset.GetParameter(connection.Conditions[i].ParameterName)
                            == null
                        )
                        {
                    ...
                    // Button
                    {
                        ...
                        if (GUILayout.Button("Add condition"))
                        {
                            ...
                            for (int i = 0; i < this.CurrentAsset.ParameterList.Count; i++)
                            {
                                ...
                                if (
                                    this.CurrentAsset.ParameterList[i].ParameterType
                                    == EditableParameter.eParamType.Int
                                )
                                {
                                    EditableIntParameter intParam =
                                        CurrentAsset.ParameterList[i] as EditableIntParameter;
                                    rightClickMenu.AddItem(
                                        new GUIContent(intParam.ParameterName),
                                        false,
                                        delegate
                                        {
                                    ...
                                    this.CurrentAsset.ParameterList[i].ParameterType
                                    == EditableParameter.eParamType.Bool
                                )
                                {
                                    EditableBoolParameter boolParam =
                                        CurrentAsset.ParameterList[i] as EditableBoolParameter;
                                    rightClickMenu.AddItem(
                                        new GUIContent(boolParam.ParameterName),
                                        false,
                                        delegate
                                        {
                                            connection.AddCondition(
                    ...
                    for (int i = 0; i < connection.Conditions.Count; i++)
                    {
                        ...
                        GUILayout.Label(
                            name,
                            GUILayout.MinWidth(conditionNameWidth),
                            GUILayout.MaxWidth(conditionNameWidth)
                        );

                        if (
                            connection.Conditions[i].ConditionType
                            == EditableCondition.eConditionType.IntCondition
                        )
                        {
                            EditableIntCondition intCond =
                                connection.Conditions[i] as EditableIntCondition;

                            intCond.CheckType = (EditableIntCondition.eCheckType)
                                EditorGUILayout.EnumPopup(intCond.CheckType);
                            intCond.RequiredValue = EditorGUILayout.IntField(intCond.RequiredValue);
                        }
                        else if (
                            connection.Conditions[i].ConditionType
                            == EditableCondition.eConditionType.BoolCondition
                        )
                        {
                            EditableBoolCondition boolCond =
                                connection.Conditions[i] as EditableBoolCondition;

                            boolCond.CheckType = (EditableBoolCondition.eCheckType)
                                EditorGUILayout.EnumPopup(boolCond.CheckType);
        ...
        private void DrawResizer()
        {
            ...
            GUILayout.BeginArea(
                new Rect(panelResizerRect.position, new Vector2(2, position.height)),
                resizerStyle
            );
            GUILayout.EndArea();
        }
        ...
        private void Panel_NodeParamActions(EditableConversationNode node)
        {
            ...
            GUILayout.Label("Set Param:", EditorStyles.boldLabel);
            {
                ...
                for (int i = 0; i < node.ParamActions.Count; i++)
                {
                    ...
                    if (
                        node.ParamActions[i].ParamActionType
                        == EditableSetParamAction.eParamActionType.Int
                    )
                    {
                        EditableSetIntParamAction intAction =
                            node.ParamActions[i] as EditableSetIntParamAction;
                        intAction.Value = EditorGUILayout.IntField(intAction.Value);
                    }
                    else if (
                        node.ParamActions[i].ParamActionType
                        == EditableSetParamAction.eParamActionType.Bool
                    )
                    {
                        EditableSetBoolParamAction boolAction =
                            node.ParamActions[i] as EditableSetBoolParamAction;
        ...
        private void ProcessInput()
        {
            ...
            switch (m_inputState)
            {
                case eInputState.Regular:
                    bool inPanel =
                        panelRect.Contains(e.mousePosition) || e.mousePosition.y < TOOLBAR_HEIGHT;
                        ...
                        for (int i = 0; i < uiNodes.Count; i++)
                        {
                            ...
                            if (uiNodes[i].rect.Contains(e.mousePosition))
                            {
                                ...
                                if (target is UIOptionNode)
                                {
                                    ...
                                    {
                                        (parent as UISpeechNode).SpeechNode.AddOption(
                                            targetOption.OptionNode
                                        );
                                    }
                                }
                                ...
                                else if (target is UISpeechNode)
                                {
                                    ...
                                    if (parent is UISpeechNode)
                                    {
                                        (parent as UISpeechNode).SpeechNode.AddSpeech(
                                            targetSpeech.SpeechNode
                                        );
                                    }
                                    else if (parent is UIOptionNode)
                                    {
                                        (parent as UIOptionNode).OptionNode.AddSpeech(
                                            targetSpeech.SpeechNode
                                        );
                                    }
                                }
        ...
        private void ProcessEvents(Event e)
        {
            ...
            switch (e.type)
            {
                case EventType.MouseDown:
                    ...
                    {
                        if (
                            DialogueEditorUtil.IsPointerNearConnection(
                                uiNodes,
                                e.mousePosition,
                                out m_connectionDeleteParent,
                                out m_connectionDeleteChild
                            )
                        )
                        {
                            GenericMenu rightClickMenu = new GenericMenu();
        ...
        private void ProcessConnectionEvents(Event e, bool inPanel)
        {
            if (uiNodes != null && !inPanel && e.type == EventType.MouseDown && e.button == 0)
            {
                ...
                bool success = DialogueEditorUtil.IsPointerNearConnection(
                    uiNodes,
                    e.mousePosition,
        ...
        public void DeleteConnection()
        {
            if (m_connectionDeleteParent != null && m_connectionDeleteChild != null)
            {
                ...
                for (int i = 0; i < m_connectionDeleteParent.Connections.Count; i++)
                {
                    ...
                    if (
                        connection is EditableSpeechConnection
                        && (connection as EditableSpeechConnection).Speech
                            == m_connectionDeleteChild
                    )
                    {
                        ...
                        connection is EditableOptionConnection
                        && (connection as EditableOptionConnection).Option
                            == m_connectionDeleteChild
                    )
                    {
                        m_connectionDeleteParent.Connections.RemoveAt(i);
        ...
        private void UnselectObject()
        {
            if (
                CurrentlySelectedObject != null
                && CurrentlySelectedObject.Type == SelectableUI.eType.Node
            )
            {
        ...
        private void Recenter()
        {
            ...
            Vector2 target = new Vector2(
                (position.width / 2) - (UISpeechNode.Width / 2) - (panelWidth / 2),
                TOOLBAR_HEIGHT + 5
            );
            Vector2 delta =
                target
                - new Vector2(ConversationRoot.EditorInfo.xPos, ConversationRoot.EditorInfo.yPos);
            for (int i = 0; i < uiNodes.Count; i++)
            {
                uiNodes[i].Drag(delta);
            }
